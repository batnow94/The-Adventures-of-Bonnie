...
//-----------------------------------------------------------------------------
/*:
 * @plugindesc (v.1.8) 'Show Text' windows float above characters and change visuals during game.
...
Window_Message.prototype.changeWindowDimensions = function() {
	if (this.pTarget != null) {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			extraFontHeight += currentFontHeight + ((up - down));
			currentFontHeight = extraFontHeight;
		}
		this.width += extraFontHeight * 40;
		extraFontHeight *= 24;
...
Window_Message.prototype.processEscapeCharacter = function(code, textState) {
	if (Galv.Mstyle.testActive) return; // if only testing the text for sizes, don't process the characters
